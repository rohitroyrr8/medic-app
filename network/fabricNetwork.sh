...
function printHelp() {
  ...
  echo "    -c <channel name> - channel name to use (defaults to \"medicchannel\")"
  ...
  echo "    -l <language> - the  language: node (default)"
  ...
  echo
  echo "	fabricNetwork.sh generate -c medicchannel"
  echo "	fabricNetwork up -c medicchannel -s couchdb"
  echo "        fabricNetwork up -c medicchannel -s couchdb -i 1.4.0"
  echo "	fabricNetwork up -l node"
  echo "	fabricNetwork down -c medicchannel"
  echo
...
function networkUp() {
  ...
  docker-compose -f docker-compose.yml -f docker-compose-couchdb.yaml up -d 2>&1
  docker ps -a
  if [ $? -ne 0 ]; then
...
function networkDown() {
  ...
  docker-compose -f docker-compose.yml -f docker-compose-couchdb.yaml down --volumes --remove-orphans
  # Don't remove the generated artifacts -- note, the ledgers are always removed
  if [ "$MODE" != "restart" ]; then
    # Bring down the network, deleting the volumes
    #Delete any ledger backups
    docker run -v "$PWD":/tmp/medicchannel --rm hyperledger/fabric-tools:"$IMAGETAG" rm -Rf /tmp/medicchannel/ledgers-backup
...
function replacePrivateKey() {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/allparticipants.medic-app.com/ca/ || exit
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR" || exit
  sed $OPTS "s/ALLPARTICIPANTS_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose.yml
...
function generateChannelArtifacts() {
  ...
  set -x
  configtxgen -profile OrdererGenesis -channelID medic-sys-channel -outputBlock ./channel-artifacts/genesis.block
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo "#################################################################"
  set -x
  configtxgen -profile MEDICCHANNEL -outputCreateChannelTx ./channel-artifacts/channel.tx -channelID "$CHANNEL_NAME"
  res=$?
  set +x
  if [ $res -ne 0 ]; then
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for allparticipantsMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile MEDICCHANNEL -outputAnchorPeersUpdate ./channel-artifacts/allparticipantsMSPanchors.tx -channelID "$CHANNEL_NAME" -asOrg allparticipantsMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for allparticipants..."
    exit 1
  fi
...
CLI_DELAY=5
# channel name defaults to "medicchannel"
...
COUCH_COMPOSE_FILE=docker-compose-couchdb.yaml
COMPOSE_FILE="${COMPOSE_FILE} -f ${COUCH_COMPOSE_FILE}";
